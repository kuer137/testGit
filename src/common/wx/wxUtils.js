...
export default (function () {
  ...
  let defTitle = '厨房电器招募活动'
  let defDesc = '德国AFI活动招募活动火热进行中'
  function getShareLink() {
    return 'http://cmms2.meicloud.com/s/wechat/index.html#/main?activityId=12&openId=43243'
    // return Config.isDev ? 'weixincs.midea.com/apps/giftCard' : 'weixin.midea.com/apps/giftCard'
  }

  function getSignature(ctx, callback) {
  ...
  function configWX(ctx, callback) {
    // var times = 0
    getSignature(ctx, function (data) {
      if (data) {
        wx.config({
          debug: false,
